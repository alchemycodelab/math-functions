...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    const sumOfThreePart1 = sum(a, b)[0];
    const sumOfThree = sum(sumOfThreePart1, c)[0];
    //  Alternately, but harder to follow: sumOfThree = sum(sum(a, b)[0], c)[0];
    const productOfThreePart1 = multiply(a, b)[0];
    const productOfThree = multiply(productOfThreePart1, c)[0];
...
export function sumArrayWithThreeNumbers(sumArr) {
    let sumTotal = sumAndMultiplyThreeNumbers(sumArr[0],sumArr[1],sumArr[2])[0];
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let multTotal = sumAndMultiplyThreeNumbers(multArr[0], multArr[1], multArr[2])[1];
    return [multTotal, `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${multTotal}.`];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    for (let i = 0; i < dynamicArray.length; i++) {
        dynamicMult = multiply(dynamicMult, dynamicArray[i])[0];
        dynamicString += dynamicArray[i];
        if (i != dynamicArray.length - 1) { 
            dynamicString += ','; 
        }
    }
    return [dynamicMult, `The numbers ${dynamicString} have a product of ${dynamicMult}.`];
}
