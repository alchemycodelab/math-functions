...
export function sumArrayWithThreeNumbers(sumArr) {
    sumArr = [2, 3, 4];
    let arrayFunction2 = sum(sumArr[0], sumArr[1]);
    let arrayFunction3 = sum(arrayFunction2[0], sumArr[2]);
    let arrayMessage = ('The numbers ' + sumArr[0] + ',' + sumArr[1] + ',' + sumArr[2] + ' have a sum of ' + arrayFunction3[0] + '.');
    return [arrayFunction3[0], arrayMessage]; 
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    multArr = [2, 3, 4];
    let arrayMultiplicationFunction2 = multiply(multArr[0], multArr[1]);
    let arrayMultiplicationFunction3 = multiply(arrayMultiplicationFunction2[0], multArr[2]);
    let arrMultMessage = ('The numbers ' + multArr[0] + ',' + multArr[1] + ',' + multArr[2] + ' have a product of ' + arrayMultiplicationFunction3[0] + '.');
    return [arrayMultiplicationFunction3[0], arrMultMessage];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    let solvedMessage = 'The numbers ';
    const solvedMessageActual = String(solvedMessage);
    let basicNumber = 1;

    for(let i = 0; i < dynamicArray.length; i++) {
        
        basicNumber = multiply(basicNumber, dynamicArray[i])[0];
        const daFunction = String(dynamicArray);
        solvedMessage = solvedMessageActual + daFunction + ' have a product of ' + basicNumber + '.';
    }
    return [basicNumber, solvedMessage];
