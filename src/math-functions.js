...
export function multiply(a, b) {
    const times = a * b;
    const string = `The product of ${a} and ${b} is ${times}.`;
    const result = [times, string];
    return result;
}
...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    const triadSum = sum(sumResult[0], c);
    const multiplyResult = multiply(a, b);
    const triadMultiply = multiply(multiplyResult[0], c);

    const stringOne = `${a} and ${b} and ${c} sum to ${triadSum[0]}.`;
    const stringTwo = `The product of ${a} and ${b} and ${c} is ${triadMultiply[0]}.`;
    const result = [triadSum[0], triadMultiply[0], stringOne, stringTwo];
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    const result = [secondSum[0], `The numbers ${sumArr[0]},${sumArr[1]},${sumArr[2]} have a sum of ${secondSum[0]}.`];
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    ...
    const secondMultiply = multiply(firstMultiply[0], multArr[2]);

    const result = [secondMultiply[0], `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${secondMultiply[0]}.`];
