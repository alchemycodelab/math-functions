...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    let added = sum(sum(a, b)[0], c)[0];
    let mult = multiply(multiply(a, b)[0], c)[0];
    return [added, mult, `${a} and ${b} and ${c} sum to ${added}.`, `The product of ${a} and ${b} and ${c} is ${mult}.`];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    let added = sum(sum(sumArr[0], sumArr[1])[0], sumArr[2])[0];
    return [added, `The numbers ${sumArr[0]},${sumArr[1]},${sumArr[2]} have a sum of ${added}.`];
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let mult = multiply(multiply(multArr[0], multArr[1])[0], multArr[2])[0];
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    let total = multiply(dynamicArray[0], dynamicArray[1])[0];
    ...
    for(let j = 0; j < dynamicArray.length - 1; j++) {

        str += `${dynamicArray[j]},`;
    }

    str += dynamicArray[dynamicArray.length - 1];

    return [total, `The numbers ${str} have a product of ${total}.`];
}
