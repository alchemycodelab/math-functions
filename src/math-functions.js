...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    const product3 = multiply(a, multiply(b, c)[0])[0];
    return [
        sum3,
        product3,
        a + ' and ' + b + ' and ' + c + ' sum to ' + sum3 + '.',
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    return [sumX, 'The numbers ' + sumArr[0] + ',' + sumArr[1] + ',' + sumArr[2] + ' have a sum of ' + sumX + '.'];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let multiplyX = multArr[0];
    for(let i = 1; i < multArr.length; i++) {
        multiplyX = multiply(multiplyX, multArr[i])[0];
    }
    return [multiplyX, 'The numbers ' + multArr[0] + ',' + multArr[1] + ',' + multArr[2] + ' have a product of ' + multiplyX + '.'];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    let multiplyX = dynamicArray[0];
    let multipliedNumbersString = dynamicArray[0];
    for(let i = 1; i < dynamicArray.length; i++) {
        multiplyX = multiply(multiplyX, dynamicArray[i])[0];
        multipliedNumbersString += ',' + dynamicArray[i];
    }
    return [multiplyX, 'The numbers ' + multipliedNumbersString + ' have a product of ' + multiplyX + '.'];
}
