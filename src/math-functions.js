...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    const sumString = `${a} and ${b} and ${c} sum to ${sumOfNums}.`;
    const proString = `The product of ${a} and ${b} and ${c} is ${productNums}.`;
    const myArray = [sumOfNums, productNums, sumString, proString];
    return myArray;
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    const sumOfNums = sum(sum(sumArr[0], sumArr[1])[0], sumArr[2]);
    const sumString = `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${sumOfNums[0]} is their sum.`;
    const myArray = [sumOfNums[0], sumString];
    return myArray;
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const equalsArray = multiply(multiply(multArr[0], multArr[1])[0], multArr[2]);
    const equalsString = `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${equalsArray[0]}.`;
    const arrayAnswer = [equalsArray[0], equalsString];
    return arrayAnswer;
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    let productOfNums = 1;
    let stringNums = 'The numbers ';
    
    for(let i = 0; i < dynamicArray.length; i++){
        productOfNums *= dynamicArray[i];
        stringNums += i + 1;
    ...
    const stringReturn = `${stringNums} have a product of ${productOfNums}.`;
    const arrayReturn = [productOfNums, stringReturn];
    return arrayReturn; 
}
