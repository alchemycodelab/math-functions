...
export function sum(a, b) {
    const total = a + b;
    const sumStatement = 'The sum of ' + a + ' and ' + b + ' is ' + total + '.';
    return [total, sumStatement];
}
...
export function multiply(a, b) {
    const product = a * b;
    const multiplyStatement = 'The product of ' + a + ' and ' + b + ' is ' + product + '.';
    return [product, multiplyStatement];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    const sumArrayAB = sum(sumArr[0], sumArr[1])[0];
    const sumArrayABC = sum(sumArrayAB, sumArr[2])[0];
    const sumArrayMessage = 'The numbers ' + sumArr[0] + ',' + sumArr[1] + ',' + sumArr[2] + ' have a sum of ' + sumArrayABC + '.';
    return [sumArrayABC, sumArrayMessage];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const prodArrayAB = multiply(multArr[0], multArr[1])[0];
    const prodArrayABC = multiply(prodArrayAB, multArr[2])[0];
    const prodArrayMessage = 'The numbers ' + multArr[0] + ',' + multArr[1] + ',' + multArr[2] + ' have a product of ' + prodArrayABC + '.';
    return [prodArrayABC, prodArrayMessage];
