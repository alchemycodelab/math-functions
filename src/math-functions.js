...
export function sum(a, b) {
    const sumResult = Number(a) + Number(b);
    const sumArray = [sumResult, 'The sum of ' + a + ' and ' + b + ' is ' + sumResult + '.'];
    return sumArray;
...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    const tripleSum = sum(a, b);
    const firstSum = tripleSum[0];
    const secondSum = sum(firstSum, c);
    const tripleSumB = secondSum[0];

    const tripleProductA = multiply(a, b);
    const firstProduct = tripleProductA[0];
    const secondProduct = multiply(firstProduct, c);
    const tripleProductB = secondProduct[0];


    const tripleArray = [tripleSumB, tripleProductB, `${a} and ${b} and ${c} sum to ${tripleSumB}.`, `The product of ${a} and ${b} and ${c} is ${tripleProductB}.`];
    return tripleArray;
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    const sumOfThreeA = sum(sumArr[0], sumArr[1]);
    const answerA = sumOfThreeA[0];
    const sumOfThreeB = sum(answerA, sumArr[2]);
    const answerB = sumOfThreeB[0];
    const tripleSumArr = [answerB, `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${answerB} is their sum.`];
    return tripleSumArr;
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const multResult1 = multiply(multArr[0], multArr[1]);
    const answer1 = multResult1[0];
    const multResult2 = multiply(answer1, multArr[2]);
    const answer2 = multResult2[0];
    const superArr = [answer2, `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${answer2}.`];
    return superArr;
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    const finalArray = [answer, `The numbers ${dynamicArray} have a product of ${answer}.`];
    return finalArray;
}
