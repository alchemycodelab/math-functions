...
const sumValue = a + b;
const addString = 'The sum of ' + a + ' and ' + b + ' is ' + sumValue + '.';
return [sumValue, addString];
...
export function sumAndMultiplyThreeNumbers(a, b, c) { 
    const sum1 = sum(a, b);
    const finalSum = sum(sum1[0], c);
    const sum2 = + a + ' and ' + b + ' and ' + c + ' sum to ' + finalSum[0] + '.';
    const mult1 = multiply(a, b);
    const finalMult = multiply(mult1[0], c);
    const prod1 = 'The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + finalMult[0] + '.';
    return [finalSum[0], finalMult[0], sum2, prod1];
}
...
export function sumArrayWithThreeNumbers(array) {
    const sumArray = sum(array[0], array[1]);
    const sumArray1 = sum(sumArray[0], array[2]);
    const sumString = + array[0] + ',' + array[1] + ',' + array[2] + ' was passed in as an array of numbers, and ' + sumArray1[0] + ' is their sum.';
    const final = [sumArray1[0], sumString];
    return final;
...
export function multiplyArrayWithThreeNumbers(multArr) { 
    const multArray = multiply(multArr[0], multArr[1]);
    const multArray1 = multiply(multArray[0], multArr[2]);
    const multString = 'The numbers ' + multArr[0] + ',' + multArr[1] + ',' + multArr[2] + ' have a product of ' + multArray1[0] + '.'; 
    const final2 = [multArray1[0], multString];
    return final2;
}
