...
export function multiply(a, b) {
    ...
    const result = [multiplied, message];
...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    const threeSum = sum(sum(a, b)[0], c)[0];
    const product = multiply(multiply(a, b)[0], c)[0];
    const sumMessage = a + ' and ' + b + ' and ' + c + ' sum to ' + threeSum + '.';
    const productMessage = 'The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + product + '.';
    const result = [threeSum, product, sumMessage, productMessage];
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    for(i = 0; i < sumArr.length; i++) {
        bigSum = sum(bigSum, sumArr[i])[0];
    }
    let result = [bigSum, 'The numbers ' + sumArr[0] + ',' + sumArr[1] + ',' + sumArr[2] + ' have a sum of ' + bigSum + '.'];
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    ...
    let result = [bigProduct, 'The numbers ' + multArr[0] + ',' + multArr[1] + ',' + multArr[2] + ' have a product of ' + bigProduct + '.'];
    return result;
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    for(let i = 0; i < dynamicArray.length; i++) {
        bigProduct = multiply(bigProduct, dynamicArray[i])[0];
        if(i > 0) { numString = numString + ',' + dynamicArray[i]; }
    }

    let result = [bigProduct, 'The numbers ' + numString + ' have a product of ' + bigProduct + '.'];
    return result;
}
