...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    const newSum = sum(firstSum[0], c);
    const firstProduct = multiply(a, b);
    const product = multiply(firstProduct[0], c);
    return [newSum[0], product[0], `${a} and ${b} and ${c} sum to ${newSum[0]}.`, `The product of ${a} and ${b} and ${c} is ${product[0]}.`];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    const finalSum = sum(firstSum[0], sumArr[2]);
    return [finalSum[0], `The numbers ${sumArr[0]},${sumArr[1]},${sumArr[2]} have a sum of ${finalSum[0]}.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const firstProduct = multiply(multArr[0], multArr[1]);
    const finalProduct = multiply(firstProduct[0], multArr[2]);
    return [finalProduct[0], `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${finalProduct[0]}.`];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    for(let i = 0; i < dynamicArray.length; i++) {  
        if(i === 0) {
            ...
            product = multiply(dynamicArray[i], product);
        } else {
            string = string + ',' + dynamicArray[i];
            product = multiply(dynamicArray[i], product[0]);
        }
    }
    return [product[0], string + ' have a product of 120.'];
}
